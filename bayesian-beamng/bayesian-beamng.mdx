...
$Y_{err}$ and $\Omega_{err}$ are a bit more complicated. I propose a sliding origin, $O$, to streamline their
derivation:
...
the start of the U-turn, pause, then follow the car back down the final straight. We can draw a vector between 
the origin and the car, $\vec{OC}$. This vector will be the basis for the following location-agnostic logic
...
We will simply append these quantities to a list at each timestep, performing the normalization and summation
at the end of a run.

Finally, a note on what will constitute one 'trial'. The authors recommend starting the car at various 
orientations for a robust evaluation of a given set of weights. The reward for each trial will be an
average over 10 episodes, with each episode slightly modifying the car's starting angle.

Here's an outline of the optimization loop:
