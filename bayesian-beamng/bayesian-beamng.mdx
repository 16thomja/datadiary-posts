...
```math
\text{EI}(x) = \left( \mu(x) - f(x^+) - \xi \right) \Phi\left( \frac{\mu(x) - f(x^+) - \xi}{\sigma(x)} \right) + \sigma(x) \phi\left( \frac{\mu(x) - f(x^+) - \xi}{\sigma(x)} \right)

\begin{itemize}
    \item Where:
    \item \( \mu(x) \) is the predicted mean at point \( x \),
    \item \( f(x^+) \) is the best observed value so far,
    \item \( \sigma(x) \) is the predicted standard deviation at point \( x \),
    \item \( \Phi \) is the cumulative distribution function (CDF) of the standard normal distribution,
    \item \( \phi \) is the probability density function (PDF) of the standard normal distribution,
    \item \( \xi \) is the exploration-exploitation trade-off parameter.
\end{itemize}
```
