...
Hierarchical Reinforcement Learning". Quite a mouthful. In it, the authors run an experiment where they train
a virtual car to take a clean U-turn at a fixed speed using the following control network:

[INSERT CONTROL NETWORK PICTURE]

You can see how, at any given time, the car's throttle and steering are goverened by 3 measurements:

1. The lateral distance between the car and the road, Y_err
2. The angle between the car's heading and the the road direction, Omega_err
...
on the road without deviating too much from a fixed speed through the duration of the turn. Let's set up our
own simulation and give this a try!
