...
$O$ will be constrained to the U-turn's line of symmetry. It will follow the car up along $\hat{y}$, stop at
the center of the U-turn, then follow the car back down the final straight. This gives us a vector between the 
origin and the car, $\vec{OC}$, that facilitates the final 2 measurements:

* $Y_{err}$ : Subtract the radius of the curve from $|\vec{OC}|$.
* $\Omega_{err}$ : Rotate $\vec{OC}$ $\frac{\pi}{2}$ clockwise to get the car's *ideal* heading. Calculate the angle
